...
def run():
    ...
    with sync_playwright() as p:
        ...
        # 4. '농장관리' 대메뉴 클릭 (서브메뉴 확장)
        log("'농장관리' 대메뉴 클릭 시도...")
        farm_menu = page.locator("text=농장관리").first
        if farm_menu.is_visible():
            farm_menu.click()
            log("'농장관리' 클릭 완료! 2초 대기...")
            time.sleep(2.0)
            
        # 5. 확장된 서브메뉴 DOM 저장
        html_content = page.content()
        html_path = os.path.join(DIAG_DIR, "03_submenus_expanded.html")
        with open(html_path, "w", encoding="utf-8") as f:
            f.write(html_content)
        page.screenshot(path=os.path.join(DIAG_DIR, "03_submenus_expanded.png"), full_page=True)
        log(f"확장된 서브메뉴 DOM 저장 완료: {os.path.basename(html_path)}")
